...
export function createRouteRecord(
    route: RouteConfig,
    params: string[] | {[k: string]: string}): Record {
  const record: Record = {
    id: route.id,
    path: route.path,
    redirect: route.redirect,
    name: route.name,
    component: route.component as () => object,
    async: route.async,
    meta: route.meta,
    props: route.props,
    params: {},
  ...
  let setParamValue = (
      key: string,
      collection: {[k: string]: string},
      index: number): void => {
    index++;
    if (index < params.length) {
      collection[key] = (params as string[])[index];
    }
  };

  // Object setter
  if (tc.isObject(params)) {
    setParamValue = (key: string, collection: {[k: string]: string}): void => {
      if (tc.not.isNullOrUndefined((params as {[k: string]: string})[key])) {
        collection[key] = (params as {[k: string]: string})[key];
      }
    };
  }
